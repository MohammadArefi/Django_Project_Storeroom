...
(function ($) {
	"use strict";
    jQuery.validator.addMethod("just_persian", function(value, element){
        return /^[\u0600-\u06FF\s]+$/.test(value);
    }," لطفا فقط به زبان فارسی بنویسید ");


    jQuery.validator.addMethod("checkidid", function(value, element) {
        return /^[0-9]{10}$/.test(value);
    }, "لطفا ده رقم وارد کنید");

    jQuery.validator.addMethod("valueNotEquals", function(value, element){
        if (value !== "default"){
            return true;
        }
        else{
            $(element + " + div.nice-select");
            return false;
        }
    }, " alert ");
	...
	var qayimaApp = {
		...
		menu_script: function() {
			...
			$submenu.prepend("<span class='menu-click'><i class='menu-arrow icon-back1'></i></span>");
        ...
        listing_todo_single: function() {
            ...
            if ($('.listing-todo-thumbnail-carousel').length) {
                $('.listing-todo-thumbnail-carousel').owlCarousel({
                    ...
                    dots: false,
                    nav: true,
                    navText: ["<span class='icon-next1'></span>", "<span class='icon-back1'></span>"],
                    responsive: {
                        280: {
                            items: 1
                        },
        ...
        megamenu_carousel: function() {
            ...
            if ($megamenu_items.length) {
                $megamenu_items.owlCarousel({
                    ...
                    dots: false,
                    nav: true,
                    navText: ["<span>بعدی</span>", "<span>قبلی</span>"],
                    responsive: {
                        280: {
                            items: 1
                        },
        ...
        add_listing_tab: function() {
            if ($('#add-listing-area').length > 0) {
                ...
                var frmMamad = $('#listmamad');
                var frmMamadValidator = frmMamad.validate({
                    rules:{
                        input_model_device: {required: true},
                        input_brand_device: {required: true},
                        input_time_device: {required: true}
                    }
                    }
                )
                ...
                var frmlocValidator = frmloc.validate({
                    ignore: [],
                    rules : {
                        input_name : {just_persian : true},
                        input_project_name : {just_persian : true},
                        input_project_id : {checkidid : true},
                    },
                });
                ...
                $('#add-listing-area').steps({
                    onChange: function (currentIndex, newIndex, stepDirection) {
                        ...
                        if (currentIndex === 1) {
                            if (stepDirection === 'forward') {
                                return frmMamad.valid();
                            }
                            if (stepDirection === 'backward') {
                                frmMamadValidator.resetForm();
                            }
                        }
